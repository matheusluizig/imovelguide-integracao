    ...
    protected function parserXml() : Void {
        ...
        foreach ($imoveis as $index => $imovel) {
            ...
            */
            ...
            $data['PrecoTemporada'] = 0; // Attr(Child): <PrecoLocacaoTemporada>. Default.
            ...
            $data['GarantiaAluguel'] = 0; // Attr(Child): <>.
            ...
            if ($imovel->has('MostrarEndereco')) {
                $data['MostrarEndereco'] = $imovel->find('MostrarEndereco')[0]->text();
            }
            ...
            $data['NomeImovel'] = "";
            $data['Novo'] = 0; // Attr(Child): <Situacao>.
            if ($imovel->has('Situacao')) {
                $data['Novo'] = $imovel->find('Situacao')[0]->text();
            }
            ...
            if ($imovel->has('UF')) {
                $data['UF'] = utf8_decode($imovel->find('UF')[0]->text()); // Attr(Child): <UF>.
            }
            ...
            $data['Numero'] = NULL; // Attr(Child): <EnderecoNumero>.
            if ($imovel->has('EnderecoNumero')) {
                $data['Numero'] = $imovel->find('EnderecoNumero')[0]->text();
            }
            ...
            $data['Latitude'] = NULL; // Attr(Child): <GMapsLatitude>.
            if ($imovel->has('GMapsLatitude')) {
                $data['Latitude'] = $imovel->find('GMapsLatitude')[0]->text();
            }

            $data['Longitude'] = NULL; // Attr(Child): <GMapsLongitude>.
            if ($imovel->has('GMapsLongitude')) {
                $data['Longitude'] = $imovel->find('GMapsLongitude')[0]->text();
            }

            $data['Video'] = NULL; // Attr(Child): <>.
            /*if (count($imovel->find('Videos'))) {
                foreach ($imovel->find('Videos')[0]->children() as $video) {
    ...
    protected function prepareXmlData() : Void {
        foreach ($this->data as $key => $imovel) {
            ...
            if (count($imovel['images'])) { // Caso seja 0 por padrão, na hora da extração de dados, significa que não tem nada, então é null pra ser inserido na DB.
                $imovel['images'] = $this->parserImageUrl($imovel['images']); // array_splice limit to 20 images.
            }
