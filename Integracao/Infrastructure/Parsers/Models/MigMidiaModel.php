...
class MigMidiaModel extends XMLBaseParser
{
    ...
    protected function parserXml(): void
    {
        ...
        foreach ($imoveis as $index => $imovel) {
            ...
            if ($imovel->has('codigoAnuncio')) {
                $codigoAnuncio = $imovel->find('codigoAnuncio');
                $data['CodigoImovel'] = count($codigoAnuncio) > 0 ? $codigoAnuncio[0]->text() : '';
            ...
            $precos = $imovel->find('precos');
            if (count($precos)) {
                foreach ($precos[0]->children() as $precoInfo) {
                    $operacao = $precoInfo->find('operacao');
                    $operationType = count($operacao) > 0 ? strtolower(trim($operacao[0]->text())) : '';
                    $quantidade = $precoInfo->find('quantidade');
                    $quanty = count($quantidade) > 0 ? $quantidade[0]->text() : '';
                    if (str_contains($operationType, 'venta')) {
                        $data['PrecoVenda'] = $quanty;
                    } elseif (str_contains($operationType, 'alquiler')) {
                        $data['PrecoLocacao'] = $quanty;
                    }
                }
            }
            ...
            if ($imovel->has('tipo')) {
                ...
                $data['TipoImovel'] = count($tipo) > 0 ? $tipo[0]->text() : 0;
            }
            ...
            if ($imovel->has('ProntoMorar')) {
                $prontoMorar = $imovel->find('ProntoMorar');
                $data['Novo'] = count($prontoMorar) > 0 ? $prontoMorar[0]->text() : NULL;
            }

            $data['AreaUtil'] = 0;
            if ($imovel->has('AreaUtil')) {
                $areaUtil = $imovel->find('AreaUtil');
                $data['AreaUtil'] = count($areaUtil) > 0 ? intval($areaUtil[0]->text()) : 0;
            }
            ...
            if ($imovel->has('ConstructionYear')) {
                $constructionYear = $imovel->find('ConstructionYear');
            ...
            if ($imovel->has('Longitude')) {
                $longitude = $imovel->find('Longitude');
                $data['Longitude'] = count($longitude) > 0 ? $longitude[0]->text() : NULL;
            ...
            if (count($caracteristicas)) {
                foreach ($caracteristicas[0]->children() as $feature) {
                    ...
                    $id = $feature->find('id');
                    $id = count($id) > 0 ? $id[0]->text() : '';
                    $nome = $feature->find('nome');
                    $name = count($nome) > 0 ? strtolower(trim($nome[0]->text())) : '';
                    $valor = $feature->find('valor');
                    $value = count($valor) > 0 ? $valor[0]->text() : '';
                    if (str_contains($name, 'area_util')) {
                        $data['AreaUtil'] = intval($value);
                    } elseif (str_contains($name, 'area_total')) {
                        $data['AreaTotal'] = intval($value);
                    } elseif (str_contains($name, 'condominio')) {
                        $data['PrecoCondominio'] = intval($value);
                    } elseif (str_contains($name, 'iptu')) {
                        $data['ValorIPTU'] = intval($value);
                    } elseif (stristr($name, 'estuda_permuta')) {
                        $data['Permuta'] = $value;
                    } elseif (str_contains($name, 'idade_do_imovel')) {
                        $data['AnoConstrucao'] = $value;
                        if (!empty($data['AnoConstrucao'])) {
                            $data['AnoConstrucao'] = Carbon::now()
                            ->subYears($data['AnoConstrucao'])->format('Y');
                        }
                    } elseif (str_contains($name, 'quarto')) {
                        $data['QtdDormitorios'] = intval($value);
                    } elseif (str_contains($name, 'suite')) {
                        $data['QtdSuites'] = intval($value);
                    } elseif (str_contains($name, 'banheiro')) {
                        $data['QtdBanheiros'] = intval($value);
                    } elseif (str_contains($name, 'garage') || str_contains($name, 'vaga')) {
                        $data['QtdVagas'] = intval($value);
                    }
                }
            }
            ...
            if ($location->has('endereco')) {
                $data['Endereco'] = $location->find('endereco')[0]->text();
            }
            ...
            if ($location->has('codigoPostal')) {
                $data['CEP'] = $location->find('codigoPostal')[0]->text();
            }

            $data['images'] = [];
                $multimedia = $imovel->find('multimidia');
                $imagesCounter = 0;
                if (count($multimedia)) {
                    if ($multimedia[0]->has('imagens')) {
                        foreach ($multimedia[0]->find('imagens')[0]->children() as $media) {
                            $image = $media->find('urlImagem');
                            if (count($image)) {
    ...
    protected function parserStatus(String $status) : Int {
        $statusTrimmed = strtolower(trim($status));
        switch ($statusTrimmed) {
            ...
            break;
            default:
                $this->adsStatusNFound[] = $statusTrimmed;
                return 0;
            break;
        }
    }
